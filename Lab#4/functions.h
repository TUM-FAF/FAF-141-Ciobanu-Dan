...
static void Paint(HWND hwnd) {
	...
	for (int i = 0; i < v_size; i++) {
		...
		for (int j = 0; j < v_size; j++) {
			// Checking the objects interactions
			if (vec[i].objInteract(vec[j].getX(), vec[j].getY())) {
				if (i != j) {
					// Conditions if the objects collide
		...
		// Changing direction on wall collision
		vec[i].reach_the_wall(rectangle.right, rectangle.bottom);
		switch (vec[i].getDirection()) {
			case 0:
				x += vec[i].getDirection() + boost;
				y -= vec[i].getDirection() + boost;
				break;
			case 1:
				x += vec[i].getDirection() + boost;
				y += vec[i].getDirection() + boost;
